...
public class DiffUtils {
	...
	return DiffUtils.diff(IOUtils.toString(_str.toURI(), Charset.defaultCharset()),
	        IOUtils.toString(_revised.toURI(), Charset.defaultCharset()));
    }
	...
	if (!patch.getDeltas().isEmpty()) {
		...
		for (int i = 1; i < patchDeltas.size(); i++) {
		    ...
		    if ((position + delta.getOriginal().size() + contextSize) >= (nextDelta.getOriginal().getPosition()
		            - contextSize)) {
